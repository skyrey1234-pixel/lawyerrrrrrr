...
const activityCode = z.enum(["COMMUNICATION", "DRAFTING", "REVIEW", "RESEARCH", "COURT", "NEGOTIATION", "ADMIN", "OTHER"]);
...
export const billingRouter = router({
  ...
  startTimer: protectedProcedure.input(z.object({ matterId: z.number().int().positive(), activityCode, narrative: z.string().trim().min(3).max(2000), sessionId: z.number().int().positive().optional() })).mutation(async ({ ctx, input }) => ({ timerId: await startBillingTimer(ctx.user.id, input) })),
  ...
  createManual: protectedProcedure.input(z.object({ matterId: z.number().int().positive(), activityCode, narrative: z.string().trim().min(3).max(2000), durationSeconds: z.number().int().positive().optional(), workDateMs: z.number().int().positive().optional() })).mutation(async ({ ctx, input }) => ({ entryId: await createBillingEntry(ctx.user.id, { matterId: input.matterId, activityCode: input.activityCode, narrative: input.narrative, durationSeconds: input.durationSeconds, durationSource: input.durationSeconds ? "manual" : "none", sourceType: "manual", sourceIdentifier: `manual:${ctx.user.id}:${Date.now()}`, workDate: input.workDateMs ? new Date(input.workDateMs) : new Date() }) })),
  createFromVoice: protectedProcedure.input(z.object({ matterId: z.number().int().positive(), sessionId: z.number().int().positive().optional(), activityCode, narrative: z.string().trim().min(3).max(2000), durationSeconds: z.number().int().positive().nullable(), sourceQuote: z.string().trim().min(3).max(4000) })).mutation(async ({ ctx, input }) => ({ entryId: await createBillingEntry(ctx.user.id, { matterId: input.matterId, sessionId: input.sessionId, activityCode: input.activityCode, narrative: input.narrative, durationSeconds: input.durationSeconds, durationSource: input.durationSeconds ? "explicit_statement" : "none", sourceType: "voice", sourceIdentifier: `voice:${input.sessionId ?? "standalone"}:${Date.now()}`, sourceQuote: input.sourceQuote }) })),
  update: protectedProcedure.input(z.object({ entryId: z.number().int().positive(), activityCode, narrative: z.string().trim().min(3).max(2000), durationSeconds: z.number().int().positive().nullable(), workDateMs: z.number().int().positive() })).mutation(async ({ ctx, input }) => { await updateBillingEntry(ctx.user.id, { entryId: input.entryId, activityCode: input.activityCode, narrative: input.narrative, durationSeconds: input.durationSeconds, workDate: new Date(input.workDateMs) }); return { success: true as const }; }),
