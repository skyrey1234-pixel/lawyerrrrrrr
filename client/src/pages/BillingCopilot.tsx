...
const activityCodes: BillingActivityCode[] = ["COMMUNICATION", "DRAFTING", "REVIEW", "RESEARCH", "COURT", "NEGOTIATION", "ADMIN", "OTHER"];
...
export default function BillingCopilot() {
  ...
  const [activityCode, setActivityCode] = useState<BillingActivityCode>("REVIEW");
  ...
  const [editActivity, setEditActivity] = useState<BillingActivityCode>("REVIEW");
  ...
  const openEdit = (record: NonNullable<typeof entries.data>[number]) => {
    ...
    setEditActivity(record.entry.activityCode as BillingActivityCode);
  ...
  const handleVoiceCommand = (transcript: string) => {
    ...
    if (command.type === "start_timer") {
      ...
      startTimer.mutate({ matterId: selectedMatterId, activityCode: command.activityCode, narrative: command.narrative });
    }
    ...
    if (command.type === "create_entry") {
      if (!selectedMatterId) { toast.error("Select a matter first."); return; }
      createFromVoice.mutate({ matterId: selectedMatterId, activityCode: command.activityCode, narrative: command.narrative, durationSeconds: command.durationSeconds, sourceQuote: command.sourceQuote });
    }
  };
  ...
  return (
    <PageFrame>
      ...
      <section className="mb-7 border border-white/10 bg-[#0a1827] p-5 sm:p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-[1fr_180px_160px_auto] xl:items-end"><div className="space-y-2"><Label>Narrative</Label><Input value={narrative} onChange={event => setNarrative(event.target.value)} className="border-white/10 bg-white/5" /></div><div className="space-y-2"><Label>Activity</Label><Select value={activityCode} onValueChange={value => setActivityCode(value as BillingActivityCode)}><SelectTrigger className="border-white/10 bg-white/5"><SelectValue /></SelectTrigger><SelectContent>{activityCodes.map(code => <SelectItem key={code} value={code}>{code}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Minutes, if known</Label><Input type="number" min="0.01" step="0.01" value={durationMinutes} onChange={event => setDurationMinutes(event.target.value)} className="border-white/10 bg-white/5" /></div><Button className="bg-[#d6b65d] text-[#07111d]" disabled={!selectedMatterId || narrative.trim().length < 3 || createManual.isPending} onClick={() => createManual.mutate({ matterId: selectedMatterId, activityCode, narrative, durationSeconds: durationMinutes ? Math.round(Number(durationMinutes) * 60) : undefined })}><Plus className="mr-2 h-4 w-4" /> Add draft</Button></div>
      </section>
