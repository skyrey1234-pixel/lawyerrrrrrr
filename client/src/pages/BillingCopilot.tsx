...
import { AlertTriangle, BadgeDollarSign, Check, CloudUpload, Download, FileClock, Mic2, Pencil, Play, Plus, ReceiptText, ShieldCheck, Square, TimerOff, X } from "lucide-react";
...
export default function BillingCopilot() {
  ...
  const integrations = trpc.integrations.settings.useQuery();
  ...
  const [syncTarget, setSyncTarget] = useState<{ entryId: number; provider: PracticeProvider } | null>(null);
  ...
  const invalidate = async () => {
    await Promise.all([utils.billing.list.invalidate(), utils.billing.activeTimer.invalidate(), utils.billing.exports.invalidate(), utils.integrations.settings.invalidate(), utils.workspace.dashboard.invalidate()]);
  };
  ...
  const syncEntry = trpc.integrations.syncEntry.useMutation({ onSuccess: async data => { await invalidate(); setSyncTarget(null); toast.success(`Billing entry synchronized as remote time entry #${data.externalRecordId}.`); }, onError: error => toast.error(error.message) });
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      needsRate: rows.filter(row => row.entry.billable && row.entry.rateStatus === "missing" && row.entry.status !== "rejected").length,
      ...
      approvedFeeCents: rows.filter(row => row.entry.status === "approved" || row.entry.status === "exported").reduce((sum, row) => sum + (row.entry.feeCents || 0), 0),
      duplicates: rows.filter(row => row.entry.duplicateOfEntryId).length,
    };
  }, [entries.data]);
  const syncRecord = syncTarget ? entries.data?.find(item => item.entry.id === syncTarget.entryId) : undefined;
  ...
  return (
    <PageFrame>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <div className="border-l-2 border-[#d6b65d]/60 bg-white/[0.03] p-5"><BadgeDollarSign className="h-4 w-4 text-[#d6b65d]" /><strong className="mt-3 block font-display text-3xl">{formatCurrencyFromCents(totals.approvedFeeCents)}</strong><span className="text-xs text-[#8f9aa7]">Approved calculated fees</span>{totals.needsRate ? <span className="mt-1 block text-[10px] text-[#dc978e]">{totals.needsRate} need a lawyer rate</span> : null}</div>
      ...
      <div className="space-y-3">{entries.data?.length ? entries.data.map(record => {
        ...
        const canSelect = entry.status === "approved";
        const selected = selectedIds.includes(entry.id);
        const hasRate = !entry.billable || (entry.rateStatus !== "missing" && entry.rateCents != null && entry.feeCents != null);
        const canApprove = Boolean(entry.durationSeconds) && hasRate;
        const directSyncEligible = ["approved", "exported"].includes(entry.status) && Boolean(entry.durationSeconds) && hasRate;
        const connectionActive = (provider: PracticeProvider) => integrations.data?.connections.some(item => item.provider === provider && item.status === "connected") ?? false;
        const synced = (provider: PracticeProvider) => integrations.data?.attempts.some(item => item.provider === provider && item.billingEntryId === entry.id && item.billingEntryRevision === entry.revision && item.status === "succeeded") ?? false;
        return (
          <article key={entry.id} className={`border bg-[#0a1827] p-4 sm:p-5 ${entry.duplicateOfEntryId ? "border-[#be8d59]/45" : "border-white/10"}`}>
            <div className="grid gap-4 xl:grid-cols-[32px_140px_minmax(0,1fr)_100px_150px_160px_auto] xl:items-center">
              <div><Checkbox checked={selected} disabled={!canSelect} onCheckedChange={checked => setSelectedIds(current => checked ? [...current, entry.id] : current.filter(id => id !== entry.id))} aria-label={`Select entry ${entry.id}`} /></div>
              <div><span className="font-mono text-[9px] uppercase tracking-[0.12em] text-[#d6b65d]">{record.matter.matterNumber}</span><p className="mt-1 text-xs text-[#8f9aa7]">{record.matter.clientName}</p></div>
              <div><div className="flex flex-wrap items-center gap-2"><strong className="text-sm">{entry.narrative}</strong>{entry.duplicateOfEntryId ? <span className="border border-[#be8d59]/40 bg-[#be8d59]/10 px-2 py-1 font-mono text-[8px] uppercase text-[#dfb27e]">Possible duplicate of #{entry.duplicateOfEntryId}</span> : null}</div><blockquote className="mt-2 line-clamp-2 text-xs italic leading-relaxed text-[#7f8b98]">{entry.sourceQuote ? `“${entry.sourceQuote}”` : `${entry.sourceType} entry · no source quotation`}</blockquote></div>
              <div><strong className={`font-mono text-lg ${entry.durationSeconds ? "text-[#f3ead8]" : "text-[#dc978e]"}`}>{entryDuration(entry.durationSeconds)}</strong><p className="text-[10px] uppercase text-[#7f8b98]">{entry.durationSource.replace("_", " ")}</p></div>
              <div><strong className={`block font-display text-lg ${hasRate ? "text-[#efe6cf]" : "text-[#dc978e]"}`}>{hasRate ? formatCurrencyFromCents(entry.feeCents, entry.currency) : "Needs rate"}</strong><p className="text-[10px] uppercase text-[#7f8b98]">{entry.rateCents == null ? "No hourly rate" : `${formatCurrencyFromCents(entry.rateCents, entry.currency)} / hr`}</p></div>
              <div><span className={`font-mono text-[9px] uppercase tracking-[0.12em] ${entry.status === "approved" || entry.status === "exported" ? "text-[#b9c8ad]" : entry.status === "rejected" ? "text-[#dc978e]" : entry.status === "needs_duration" ? "text-[#e2a097]" : "text-[#d6b65d]"}`}>{entry.status.replace("_", " ")}</span><p className="mt-1 text-[10px] text-[#7f8b98]">{new Date(entry.workDate).toLocaleDateString()} · {record.billingCode ? `${record.billingCode.code} · ${record.billingCode.label}` : `${entry.activityCode} · legacy`}</p></div>
              <div className="flex max-w-[250px] flex-wrap justify-start gap-2 xl:justify-end"><Button size="sm" variant="outline" className="border-white/12 bg-transparent" disabled={entry.status === "exported"} onClick={() => openEdit(record)}><Pencil className="h-3.5 w-3.5" /></Button>{["draft", "needs_duration"].includes(entry.status) ? <><Button size="sm" className="bg-[#d6b65d] text-[#07111d]" title={!entry.durationSeconds ? "Add verified duration" : !hasRate ? "Assign an effective lawyer rate" : "Approve entry"} disabled={!canApprove || reviewEntry.isPending} onClick={() => reviewEntry.mutate({ entryId: entry.id, decision: "approved" })}><Check className="h-3.5 w-3.5" /></Button><Button size="sm" variant="outline" className="border-white/12 bg-transparent" disabled={reviewEntry.isPending} onClick={() => reviewEntry.mutate({ entryId: entry.id, decision: "rejected" })}><X className="h-3.5 w-3.5" /></Button></> : null}{(["clio", "mycase"] as const).map(provider => synced(provider) ? <span key={provider} className="border border-[#b9c8ad]/30 bg-[#b9c8ad]/8 px-2 py-1 font-mono text-[8px] uppercase text-[#b9c8ad]">{provider} synced</span> : <Button key={provider} size="sm" variant="outline" className="border-[#d6b65d]/25 bg-transparent px-2 text-[10px]" title={!connectionActive(provider) ? `${provider === "clio" ? "Clio" : "MyCase"} is not connected` : !directSyncEligible ? "Approve this rate-complete entry first" : `Review and send to ${provider === "clio" ? "Clio" : "MyCase"}`} disabled={!connectionActive(provider) || !directSyncEligible} onClick={() => setSyncTarget({ entryId: entry.id, provider })}><CloudUpload className="mr-1 h-3 w-3" /> {provider === "clio" ? "Clio" : "MyCase"}</Button>)}</div>
            </div>
          </article>
        );
      ...
      <Dialog open={Boolean(syncTarget)} onOpenChange={open => { if (!open) setSyncTarget(null); }}>
        <DialogContent className="border-[#d6b65d]/25 bg-[#0b1725] text-[#f7f1e5] sm:max-w-xl">
          <DialogHeader>
            <DialogTitle className="font-display text-3xl">Confirm external billing sync</DialogTitle>
            <DialogDescription className="text-[#97a2ae]">This is a real external action. CounselScribe will create one time entry in {syncTarget?.provider === "clio" ? "Clio Manage" : "MyCase"} using the approved, snapshotted values below.</DialogDescription>
          </DialogHeader>
          {syncRecord ? <div className="space-y-3 py-3 text-sm"><div className="border border-white/10 bg-white/[0.03] p-4"><span className="font-mono text-[9px] uppercase tracking-[0.12em] text-[#d6b65d]">{syncRecord.matter.matterNumber} · {syncRecord.matter.clientName}</span><strong className="mt-2 block">{syncRecord.entry.narrative}</strong></div><div className="grid gap-3 sm:grid-cols-3"><div className="border border-white/10 p-3"><span className="text-[10px] uppercase text-[#84909d]">Exact time</span><strong className="mt-1 block">{entryDuration(syncRecord.entry.durationSeconds)}</strong></div><div className="border border-white/10 p-3"><span className="text-[10px] uppercase text-[#84909d]">Hourly rate</span><strong className="mt-1 block">{formatCurrencyFromCents(syncRecord.entry.rateCents, syncRecord.entry.currency)}</strong></div><div className="border border-white/10 p-3"><span className="text-[10px] uppercase text-[#84909d]">Calculated fee</span><strong className="mt-1 block text-[#d6b65d]">{formatCurrencyFromCents(syncRecord.entry.feeCents, syncRecord.entry.currency)}</strong></div></div><p className="border-l-2 border-[#d6b65d] bg-[#d6b65d]/7 p-3 text-xs leading-relaxed text-[#d9ca9f]">The provider matter, lawyer, and activity mappings must already exist. A revision-specific idempotency record prevents this exact entry revision from being posted twice.</p></div> : <p className="py-4 text-sm text-[#dc978e]">The billing entry is no longer available.</p>}
          <DialogFooter><Button variant="outline" className="border-white/15 bg-transparent" onClick={() => setSyncTarget(null)}>Cancel</Button><Button className="bg-[#d6b65d] text-[#07111d]" disabled={!syncTarget || !syncRecord || syncEntry.isPending} onClick={() => syncTarget && syncEntry.mutate({ provider: syncTarget.provider, billingEntryId: syncTarget.entryId, confirmed: true })}><CloudUpload className="mr-2 h-4 w-4" /> Confirm and send</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </PageFrame>
  );
